...
!  standalone build::
!
!     f2py -c -m _reconutils _reconutils.f90 --opt="-O3" --f90exec=gfortran
!           -L$LAPACK_DIR -llapack -lrefblas -lgfortran
!
!  ==============================================================================
...
!  =============================================================================
   subroutine pixelxpixelsolver(metfile,numsc,doline,docol,addr,lenaddr,mvec)
!
!  Estimate the posterior model using a pixel x pixel (Cm^-1 = 0) implementation
!
!  Parameters
!  ----------
!  metfile  :  string
...
!  Outputs
!  -------
!  model    :  1d float array
!              posterior model inferred from Lapack weighted least squares 
!              routine dggglm; estimated pixel x pixel
!
   implicit none
   character*1    delm
   character*250  metfile
   integer        numsc, doline, docol, lenaddr
   integer        fids(numsc,3), cols(numsc), jind, aind, gind
   integer        i,j,m,nchk,cnt,latlon(numsc,5),mind
   integer        addr(lenaddr)
   real*4         mvec(3*lenaddr), nulls(numsc,5)
   real*4         unw(numsc,docol),cor(numsc,docol),los(numsc,3*docol)
   real*8         corsq,gmat(numsc,3),cdainv(numsc),dvec(numsc),modv(3)
   character(len=250),dimension(numsc,3)::  filenms
   character(len=11),parameter:: fu='unformatted'
   character(len=6), parameter:: di='direct'

!f2py intent(in) mefile,numsc,doline,docol,addr,lenaddr
!f2py intent(out) mvec
   ...
   aind = 0  ! aind is zero-indexed for python
   do i=1,doline
      call print_status(i,doline,2)
      call get_data_vecs(unw,cor,los,numsc,docol,fids,cols,nulls(:,1:3),latlon,i,cnt)
      do j=1,docol
         if (addr(aind+1).eq.(i-1)*docol+j-1) then
            jind = 3*(j-1)+1
            gind = 0; gmat = 0.d0
            cdainv = 0.d0; dvec = 0.d0
            do m=1,numsc
               call interogate_data(unw(m,j),cor(m,j),los(m,jind:jind+2),nulls(m,1:3),nchk)
               if (nchk.eq.1) then
                  gind = gind + 1
                  corsq = dble(cor(m,j))**2
                  gmat(gind,1:3) = dble(los(m,jind:jind+2))
                  cdainv(gind) = dble(nulls(m,4))*corsq/(1.d0-corsq)
                  dvec(gind) = dble(unw(m,j)) * dble(nulls(m,5))
               endif
            enddo
            call wlsq(gmat(:gind,:),cdainv(:gind),dvec(:gind),gind,modv)
            mind = 3*aind+1
            mvec(mind:mind+2) = sngl(modv(1:3))
            aind = aind + 1
            if (aind.eq.lenaddr) aind = aind - 1
         endif
      enddo
   enddo
   print *,' '    
...
!  ==============================================================================
   ...
   real*4               los(numsc,3*docol),dumv1(100*docol)
...
!  ==============================================================================
   subroutine wlsq(gmat,cdainv,dvec,gind,modv)
!  calls dggglm : lapack routine (double) for general weighted least squares
!  Inputs
!  ------
!  gmat     :     double; [gind x 3]
!                 G matrix (LOS unit vectors)
!  cdainv   :     double; [gind x 1]
!                 diagonal components of C_d^-1
!  dvec     :     double; [gind x 1]
!                 InSAR data
!  gind     :     int
!           :     number of rows in gmat
!
!  Outputs  
!  -------
!  modv     :     double; [3 x 1]
!           :     model vector; results of least sq. inverstion
!
   implicit none
   integer     gind,lwork,info,i
   real*8      gmat(gind,3),cdainv(gind),dvec(gind),modv(3)
   real*8      cd(gind,gind),dyvec(gind),work(3+gind+gind)
!f2py intent(in)  gmat,cdainv,dvec,numsc,gind
!f2py intent(out) modv
   lwork = 3+gind+gind
   cd = 0.d0
   do i=1,gind
      cd(i,i) = 1.d0/cdainv(i)  !  dggglm inverts cd...so we need to invert the inverse
   enddo
   call dggglm(gind,3,gind,gmat,gind,cd,gind,dvec,modv,dyvec,work,lwork,info)
   if (info.ne.0) then
      print *,'dggglm exit info = ',info
      print *,'setting associated model values to -99.9'
      modv = -99.9
   endif 
   end subroutine

!  ==============================================================================
   subroutine dinv(mat,m,n)
!  returns inv(mat) for a double precision [m x n] matrix mat 
   implicit none
   integer        n,m,info,ipiv(n)
   real*8         mat(m,n), work(n*n)
!f2py intent(in) m,n
!f2py intent(in/out) mat
   if (m.eq.n) then
      call dgetrf(n,n,mat,n,ipiv,info)
      call dgetri(n,mat,n,ipiv,work,n*n,info)
   else
      print *,"matrix is not square, returning unmodified mat"
   endif
   end subroutine
